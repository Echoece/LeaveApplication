...
public class JwtTokenVerifier extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        String authorizationHeader = request.getHeader(jwtConfig.getAuthorizationHeader());     //get the token from header

        // if authorization header is null or empty, or it doesnt start with Bearer prefix, the request will be rejected in
        // filter-chain as we didnt set the authentication.
        if(Strings.isNullOrEmpty(authorizationHeader) || !authorizationHeader.startsWith(jwtConfig.getTokenPrefix())){
            filterChain.doFilter(request,response);
            return;
        }

        String token = authorizationHeader.replace(jwtConfig.getTokenPrefix(), "");   // extract the JWT from header
